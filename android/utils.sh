...
function wait_emulator_to_be_ready () {
  ...
  while [ "$boot_completed" == false ]; do
    ...
    echo "Waiting for emulator to be ready. Boot Status: $status"
...
function install_mock_server_certificate() {
    hash=$(openssl x509 -inform PEM -subject_hash_old -in $MOCK_SERVER_CERTIFICATE | head -1)
    OUT_FILE_NAME="$hash.0"

    cp $MOCK_SERVER_CERTIFICATE $OUT_FILE_NAME
    openssl x509 -inform PEM -text -in $MOCK_SERVER_CERTIFICATE -out /dev/null >> $OUT_FILE_NAME

    echo "Install mock server certificate"
...
install_google_play
sleep 1
install_mock_server_certificate
wait_emulator_to_be_ready
adb install /config/sample_app.apk
sleep 1
